        ...
        for(int i=0; i < 14; i++)
        {
            for(int j = 0; j<15; j++)
            {
                if(levelMap[j,i] == 1)
                {
                    ...
                    if(j == 14 && i == 0)
                    {
                        g.transform.position = new Vector3(-13.256f,7.3f);
                    }
                    if (j == 5 && i == 0)
                    {
                        g.transform.position = new Vector3(-13.304f, -1.223f);
                        g.transform.Rotate(0, 0, 90, 0);
                    }
                    if (j == 5 && i == 5)
                    {
                        g.transform.position = new Vector3(-8.74f, -1.74f);
                        g.transform.Rotate(0, 0, 270, 0);
                    }
                    if (j == 1 && i == 5)
                    {
                        g.transform.position = new Vector3(-8.78f, -5.262f);
                ...
                {
                    GameObject g = new GameObject();
                    g.transform.position = new Vector3(-0.5f, 7.383f);
                    var s = g.AddComponent<SpriteRenderer>();
                    s.sprite = Tjunction;
                }
            }
